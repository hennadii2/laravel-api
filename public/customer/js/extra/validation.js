  ...
  return true;
	...
	function validateFrm(tmpVar)
	{
			...
			for(i = 0; i < elements.length; ++i)
			{
				...
				if(myString.substring(0, 3) == 'm__')
				{
					...
					if(field.type=='select-one') {
						...
						strMessage = 'Please browse a '+strMsg+'.';
					}
					else {

						strMessage = 'Please enter a '+strMsg+'.';
					}
                ...
                if(field.type == 'file' && field.name != "cam_img"  && field.value!='')
				{
				...
				if(field.type == 'file' && field.name == "cam_img"  && field.value!='')
				{
					if(invalidCamFileFormat(field, 'Please upload only png and jpg.'))
					{
						return false;
					}
				}
	 ...
	 function addMessage(field1, message)
	 {
		...
		div.innerHTML = "<div style='color:#FF0000;font-family:verdana;font-size:11px;font-weight:bold;'>"+message+"</div>";
...
/***************************************************************/
	...
	*/
    ...
    function invalidCamFileFormat(field, message)
	{
		if (field.value != "") {
			myString = new String(field.value);
			start=myString.lastIndexOf(".");
			argvalue = myString.substring(start, myString.length);
			if(argvalue.toLowerCase() != ".png" && argvalue.toLowerCase() != ".jpg" ) {
				addMessage(field, message);
				return true;
			}
		}
