...
<div class="page-breadcrumb mb-30"> <a href="{{URL::to('/')}}/super_admin/index">Setting</a>/<span>Product Categories</span> </div>
...
<td> </td>
<td><a href="{{URL::to('/')}}/super_admin/product_type/{{$value->type_id}}" class="tt_green_text"><i class="fa fa-pencil"></i> Edit</a> |
<a href="{{URL::to('/')}}/super_admin/type_delete/{{$value->type_id}}" class="tt_green_text red_text"><i class="fa fa-trash"></i> Delete</a></td>
</tr>
<?php
$sub_category=DB::table('admin_type')->where('parentid','=', $value->type_id)->get();
foreach($sub_category as $key1 => $sub){?>

<tr>
<td>{{$sub->type_name}} </td>
<td> {{$value->type_name}}</td>
<td><a href="{{URL::to('/')}}/super_admin/product_type/{{$sub->type_id}}" class="tt_green_text"><i class="fa fa-pencil"></i> Edit</a> |
<a href="{{URL::to('/')}}/super_admin/type_delete/{{$sub->type_id}}" class="tt_green_text red_text"><i class="fa fa-trash"></i> Delete</a></td>
</tr>
<?php }?>
...
<select name="category_name" id="category_name" class="input-group-lg  full_input" style="max-width: 100%; !important;" >
...
<option value="NULL">NONE </option>
@foreach($main_cat as $key => $value)
<option value="{{$value->type_id}}" {{ $parentid == $value->type_id ? 'selected="selected"' : '' }}>{{$value->type_name}}</option>
@endforeach
</select>
